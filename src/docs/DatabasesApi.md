...
> Database get_database_by_name(name, org_id)
...
with wodby.ApiClient(configuration) as api_client:
    ...
    try:
        # Get database by name
        api_response = api_instance.get_database_by_name(name, org_id)
...
# **list_databases**
> List[Database] list_databases(org_id, project_ids=project_ids, kind=kind)
...
with wodby.ApiClient(configuration) as api_client:
    ...
    try:
        # List databases
        api_response = api_instance.list_databases(org_id, project_ids=project_ids, kind=kind)
