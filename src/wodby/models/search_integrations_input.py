...
class SearchIntegrationsInput(BaseModel):
    """
    ...
    def types_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['GIT', 'STORAGE', 'KUBERNETES', 'DB', 'VARIABLE', 'REGISTRY', 'NFS', 'CI', 'SMTP', 'APPLICATION_ACCESS', 'VPN']):
                raise ValueError("each list item must be one of ('GIT', 'STORAGE', 'KUBERNETES', 'DB', 'VARIABLE', 'REGISTRY', 'NFS', 'CI', 'SMTP', 'APPLICATION_ACCESS', 'VPN')")
        return value
